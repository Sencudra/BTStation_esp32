...
void setup()
{
  ...
  if (!btName || btName.length() <= 0) {
    #ifdef DEBUG
      Serial.println("Bluetooth name not set");
    #endif
    uint8_t mac[6];
    esp_read_mac(mac, ESP_MAC_BT);
    btName = String("?BtStation-") + String(mac[4], HEX) + String(":") + String(mac[5], HEX);
  }
...
void updateTeamMask()
{
  ...
  */
  ...
  // неправильный тип чипа
  if (ntag_page[2] != ntagMark) //NTAG_MARK
  {
    RfidEnd();
#ifdef DEBUG
      Serial.println(F("!!!incorrect chip"));
...
void BTConfirmRequestCallback(uint32_t numVal)
{
  ...
  bool confirmedByButton = !digitalRead(BT_CONFIRM_PIN);
